...
CONFIG_FILE_TMP="/tmp/side-route_tmp"
...
# 所以有两个方案:
# 1. 添加procd_set_param file "$CONFIG_FILE",注释该方法  这会使用procd,他会重启daemon
# 2. 删除procd_set_param file "$CONFIG_FILE",解注该方法  daemon中trap sighup方法,不会重启daemon
reload_service() {
    # 重新载入配置文件    logger -t Side-route "Reload_service Reloading configuration"
