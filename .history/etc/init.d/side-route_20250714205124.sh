...
start() {
    ...
    cp /etc/config/side-route /var/etc/side-route

    procd_open_instance
    procd_set_param command /usr/bin/lua /usr/sbin/ride-route-daemon.lua
    procd_set_param pidfile /var/run/ride-route.pid
