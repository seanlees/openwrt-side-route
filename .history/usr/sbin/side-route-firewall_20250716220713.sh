...
sync_device_nft_rules() {
    ...
    if [ "$fwmark" != "$fwmark_old" ]; then
        ...
        echo "$NEW_DEVICES" | while IFS='|' read -r new_ip new_ip new_enable; do
    ...
    ############### 处理配置文件的变化  ###############
    # 3. 处理删除的设备和IP变化 
    echo "$OLD_DEVICES" | while IFS='|' read -r section old_ip old_enable; do
        local new_line=$(echo "$NEW_DEVICES" | grep "^$section|")
        if [ -z "$new_line" ]; then
            delete_nft_rule "$old_ip" "$fwmark_old"
        else
            IFS='|' read -r _ new_ip new_enable <<EOF
$new_line
EOF
            if [ "$old_ip" != "$new_ip" ] || [ "$new_enable" = "0" ]; then
                delete_nft_rule "$old_ip" "$fwmark_old"
            fi
        fi
    done
...
$old_line
EOF
            if [ "$old_ip" != "$new_ip" ] && [ "$new_enable" = "1" ]; then
                add_or_update_nft_rule "$new_ip" "$fwmark"
            elif [ "$old_enable" = "0" ] && [ "$new_enable" = "1" ]; then
                add_or_update_nft_rule "$new_ip" "$fwmark"
            fi
        fi
    done
