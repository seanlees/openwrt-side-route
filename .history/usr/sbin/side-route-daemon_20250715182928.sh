...
reload_config() {
    ...
    ROUTE_FWMARK=$(uci -q get $CONFIG_NAME.global.route_mark)
    ...
    ROUTE_TABLE_NUM_OLD=$(uci -q get $TMP_CONFIG.global.route_table_num)
    ROUTE_SITE_IP_OLD=$(uci -q get $TMP_CONFIG.global.route_side_ip)
    ROUTE_INTERFACE_OLD=$(uci -q get $TMP_CONFIG.global.route_interface)
    ROUTE_FWMARK_OLD=$(uci -q get $TMP_CONFIG.global.route_mark)
    CHECK_TIMEOUT=$(uci -q get $TMP_CONFIG.global.check_timeout)
    ...
    if [ "$ROUTE_TABLE_NUM" != "$ROUTE_TABLE_NUM_OLD" ] || [ "$ROUTE_SIDE_IP" != "$ROUTE_SITE_IP_OLD" ] || \
       [ "$ROUTE_INTERFACE" != "$ROUTE_INTERFACE_OLD" ] || [ "$ROUTE_FWMARK" != "$ROUTE_FWMARK_OLD" ] ]; then
        ...
        update_routing_table "update"
    else
        if [ -z "$RELOAD_FIRST" ]; then
         update_routing_table "init"
         RELOAD_FIRST="1"
        fi
    fi
...
update_routing_table() {
    ...
    # 5. 添加新规则
    if ip rule add fwmark $ROUTE_FWMARK lookup $table_num; then
        log "info" "create new rule: fwmark $ROUTE_FWMARK lookup $table_num"
    else
    ...
    # 6. 添加默认路由
    if ip route replace default via $gateway dev $interface table $table_num; then
