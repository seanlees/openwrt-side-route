...
sync_device_nft_rules() {
    local new_config="/etc/config/side-route"
    local old_config="/tmp/side-route"
    local fwmark="$1"

    local old_devs=$(parse_devices "$old_config" "device")
    local new_devs=$(parse_devices "$new_config" "device")
