...
update_nft_rules() {
    ...
    for ip in $nft_rules; do
        if ! echo "$NEW_DEVICES" | grep -q "^$ip|"; then
        log "info" "Removing obsolete rule: ip saddr $ip"
        
        # 获取该IP对应的所有规则句柄
        handles=$(nft --handle list chain inet $table_name $chain_name 2>/dev/null | \
            grep -A2 "ip saddr $ip" | \
            awk '/# handle/{print $NF}')
        
        # 逐个删除规则
        for handle in $handles; do
            nft delete rule inet $table_name $chain_name handle "$handle" 2>/dev/null || true
        done
    fi
    done
