...
rule_exists() {
    local expr="$1"
    log "debug" "Checking rule: $expr"
    # nft list ruleset | grep -A1 -B1 "$expr" | grep -q "$expr" 2>/dev/null
    # nft list chain inet "$table_name" "$chain_name" 2>/dev/null | grep -qF "$expr"
    if nft list chain inet "$table_name" "$chain_name" 2>/dev/null | grep -qF "$expr"; then
        log "debug" "Rule found: $expr"
        return 0
    else
...
delete_nft_rule() {
    ...
    if rule_exists "$expr"; then
        ...
        local handle=$(nft --handle list chain inet "$table_name" "$chain_name" | grep -A2 "$expr" | awk '/# handle/{print $NF}' 2>/dev/null)
        if [ -n "$handle" ]; then
...
update_nft_rules() {
    ...
    if [ "$fwmark" != "$fwmark_old" ]; then
        log "info" "1. Firewall mark changed from $fwmark_old to $fwmark"
    ...
    if [ "$NEW_DEVICES" = "$OLD_DEVICES" ] && [ "$fwmark" = "$fwmark_old" ]; then
        log "debug" "2. config not change, checking nft rules exists"
        echo "$NEW_DEVICES" | while IFS='|' read -r new_ip new_enable; do
    ...
    ############### 处理配置文件的变化  ###############
    # 3. 处理删除的设备和IP变化 
    log "debug" "Device delete OR ip change"
...
EOF
    ...
    log "debug" "Device add OR ip change"
    # 4. 处理新增设备和IP变化
    echo "$NEW_DEVICES" | while IFS='|' read -r new_ip new_enable; do
...
EOF
    ...
    # 清理错误规则，new_devices中不存在但存在于nft中的规则
    log "debug" "5. clean error rules"
    local nft_rules=$(nft list chain inet $table_name $chain_name 2>/dev/null | grep -o 'ip saddr [0-9\.]\+' | cut -d ' ' -f3)
    if [ -z "$nft_rules" ]; then
        log "debug" "No existing rules to clean up"
        return 0
    fi
    for ip in $CURRENT_DEVICES; do
        echo "$NEW_DEVICES" | grep -qw "$ip" || {
            log "info" "Removing obsolete rule: ip saddr $ip"
            nft --handle list chain inet $table_name $chain_name  |grep "$ip" | awk '/handle/{print $NF}' | xargs  nft delete rule inet $table_name $chain_name handle  # 清理错误规则，new_devices中不存在但存在于nft中的规则
    log "debug" "clean error rules"
    local nft_rules=$(nft list chain inet $table_name $chain_name 2>/dev/null | grep -o 'ip saddr [0-9\.]\+' | cut -d ' ' -f3)
    if [ -z "$nft_rules" ]; then
        log "debug" "No existing rules to clean up"
        return 0
    ...
    for ip in $CURRENT_DEVICES; do
        echo "$NEW_DEVICES" | grep -qw "$ip" || {
            log "info" "Removing obsolete rule: ip daddr $ip"
            nft --handle list chain inet $table_name $chain_name  |grep "$ip" | awk '/handle/{print $NF}' | xargs  nft delete rule inet $table_name $chain_name handle 
        }
    done
        }
    done
