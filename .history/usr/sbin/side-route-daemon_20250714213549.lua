...
function reload_config()
    ...
    cursor:foreach("side-route", "device", function(section)
        if section.scientific == "1" then
            local ip = devicesIp[section.name]
            local cmd = string.format("ip route add %s dev %s table %s", ip, route_interface, route_table_name)
            os.execute(cmd)
            cmd = string.format("ip rule add fwmark %s lookup %s", route_mark, route_table_name)
            os.execute(cmd)
        end
