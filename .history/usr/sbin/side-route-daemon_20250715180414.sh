...
reload_config() {
    ...
    local ROUTE_TABLE_NUM_OLD=$(uci -q get $TMP_CONFIG.global.route_table_num)
    local route_side_ip_tmp=$(uci -q get $TMP_CONFIG.global.route_side_ip)
    local route_interface_tmp=$(uci -q get $TMP_CONFIG.global.route_interface)
    local route_mark_tmp=$(uci -q get $TMP_CONFIG.global.route_mark)
    local check_timeout_tmp=$(uci -q get $TMP_CONFIG.global.check_timeout)
    
    if [ "$ROUTE_TABLE_NUM" != "$ROUTE_TABLE_NUM_OLD" ] || [ "$ROUTE_SIDE_IP" != "$route_side_ip_tmp" ] || \
       [ "$ROUTE_INTERFACE" != "$route_interface_tmp" ] || [ "$route_mark" != "$route_mark_tmp" ] ]; then
