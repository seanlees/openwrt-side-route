...
ensure_nft_table_chain() {
    if ! nft list tables inet | grep -q "table inet $table_name"; then
        fw_log "info" "Creating nft table: $table_name"
        nft add table inet "$table_name"
    fi

    if ! nft list chains inet "$table_name" | grep -q "$chain_name"; then
        fw_fw_log "info" "Creating nft chain: $chain_name"
...
add_or_update_nft_rule() {
    ...
    if ! rule_exists "$expr"; then
        fw_log "info" "Adding nft rule: $expr"
...
delete_nft_rule() {
    ...
    local expr=$(get_rule_expression "$ip" "$fwmark")
    if rule_exists "$expr"; then
        fw_log "info" "Deleting nft rule: $expr"
...
parse_devices() {
    ...
    # 使用config_load加载配置
    config_load "$config" || {
        fw_log "error" "Failed to load config: $config"
        return 1
    }
...
update_nft_rules() {
    ...
    if [ "$fwmark" != "$fwmark_old" ]; then
        fw_log "info" "Firewall mark changed from $fwmark_old to $fwmark"
