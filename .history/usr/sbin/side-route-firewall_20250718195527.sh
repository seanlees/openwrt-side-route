...
update_nft_rules() {
    ...
    log "debug" "0. clean error rules"
    local nft_rules=$(nft list chain inet $table_name $chain_name 2>/dev/null | \
    grep -A2 'ip saddr' | \
    awk '/ip saddr/{ip=$3} /meta mark set/{print ip}' | \
    sort -u)

    if [ -z "$nft_rules" ]; then
        log "debug" "No existing rules to clean up"
    fi
    for ip in $nft_rules; do
        echo "$NEW_DEVICES" | grep -qw "$ip" || {
            log "info" "Removing obsolete rule: ip saddr $ip"
            nft --handle list chain inet $table_name $chain_name  \
                |grep "$ip" | awk '/handle/{print $NF}' | xargs  nft delete rule inet $table_name $chain_name handle 2>/dev/null || true
        }
    done
