...
reload_config() {
    ...
    local route_mark_tmp=$(uci -q get $TMP_CONFIG.global.route_mark)
    ...
    if [ "$ROUTE_TABLE_NUM" != "$ROUTE_TABLE_NUM_OLD" ] || [ "$ROUTE_SIDE_IP" != "$ROUTE_SITE_IP_OLD" ] || \
       [ "$ROUTE_INTERFACE" != "$ROUTE_INTERFACE_OLD" ] || [ "$route_mark" != "$route_mark_tmp" ] ]; then
...
update_routing_table() {
    ...
    if ! grep -qE "^$ROUTE_TABLE_NUM $ROUTE_TABLE_NUM" /etc/iproute2/rt_tables; then
        ...
        # 如果旧表存在且不同，删除旧表
        if [ -n "$ROUTE_TABLE_NUM_OLD" ] && [ "$ROUTE_TABLE_NUM_OLD" != "$ROUTE_TABLE_NUM" ] && grep -q "^$ROUTE_TABLE_NUM_OLD\s" /etc/iproute2/rt_tables; then
