...
EOF
    ...
    for ip in $CURRENT_DEVICES; do
        echo "$NEW_DEVICES" | grep -qw "$ip" || {
            ...
            nft delete rule inet $table_name $chain_name "ip saddr $ip"
            nft --handle list chain inet $table_name $chain_name  |grep 192.168.28.159 | awk '/handle/{print $NF}' | xargs  nft delete rule inet $table_name $chain_name handle 
        }
    done
}
