...
local function handle_signal(sig)
    if sig == nixio.const.SIGHUP then
        nixio.syslog("info", "Received SIGHUP, reloading config")
        local status, err = pcall(reload_config)  
        if not status then
            nixio.syslog("err", "Reload failed: "..tostring(err))
        end
...
-- 初始加载配置
reload_config()
sys.exec("11111111111111111111")

-- 主循环（可选）
while true do
    nixio.poll(nil, -1)  -- 60秒超时
    -- 定期维护任务
