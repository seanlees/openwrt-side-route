...
delete_nft_rule() {
    ...
    if rule_exists "$expr"; then
        ...
        nft --handle list chain inet $table_name mangle_prerouting  |grep 192.168.28.159 | awk '/handle/{print $NF}' | xargs  nft delete rule inet fw4 mangle_prerouting handle 
    fi
}
