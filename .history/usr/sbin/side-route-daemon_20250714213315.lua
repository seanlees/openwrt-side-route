...
function reload_config()
    ...
    -- 读取配置文件
    local config = uci.load("side-route")
    -- 获取全局配置
    local global = config:get_all("global")
    -- 获取设备列表
    local devices = config:get_all("device")
    -- 获取路由表ID
    local route_table_id = global.route_table_name
    -- 获取旁路由IP
    ...
    cursor:foreach("side-route", "global", function(section)
        local device_name = section.route_table_name
