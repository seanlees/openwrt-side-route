...
reload_config() {
    ...
    CHECK_TIMEOUT=$(uci -q get $CONFIG_FILE_TMP.global.check_timeout)
    RELOAD_FIRST=$(uci -q get $CONFIG_FILE_TMP.global.reload_first || echo "") 
    ...
    if [ "$ROUTE_TABLE_NUM" != "$ROUTE_TABLE_NUM_OLD" ] || [ "$ROUTE_SIDE_IP" != "$ROUTE_SITE_IP_OLD" ] || \
       [ "$ROUTE_INTERFACE" != "$ROUTE_INTERFACE_OLD" ] || [ "$ROUTE_FWMARK" != "$ROUTE_FWMARK_OLD" ]; then
        ...
        if [ -z "$RELOAD_FIRST" ]; then
            ...
            update_routing_table
            RELOAD_FIRST="1"
            uci set $CONFIG_FILE_TMP.global.reload_first="1"
            uci commit $CONFIG_FILE_TMP
        fi
    fi
