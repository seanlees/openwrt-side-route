...
reload_config() {
    ...
    if [ "$route_table_num" != "$route_table_num_tmp" ] || [ "$route_side_ip" != "$route_side_ip_tmp" ] || [ "$route_interface" != "$route_interface_tmp" ] || [ "$route_mark" != "$route_mark_tmp" ] || [ "$check_timeout" != "$check_timeout_tmp" ]; then
        ...
        if ! grep -q "^$route_table_num_tmp\t$route_table_num_tmp" /etc/iproute2/rt_tables; then
            echo "$route_table_num_tmp\t$route_table_num_tmp" >> /etc/iproute2/rt_tables  
            log "info" "create route table $route_table_num_tmp"
        fi
        ...
        # 3. 删除旧路由表（如果存在）
        ip route flush table $route_table_num_tmp 2>/dev/null && \
        log "info" "clean route table $route_table_num_tmp"

        # 4. 添加新规则
        ip rule add fwmark $fwmark lookup $table_num
        if [ $? -eq 0 ]; then
