...
update_routing_table() {
    ...
    if ! grep -qE "^$table_num $table_num" /etc/iproute2/rt_tables; then
        ...
        # 如果旧表存在且不同，删除旧表
        if [ -n "$old_table_num" ] && [ "$old_table_num" != "$table_num" ] && grep -q "$old_table_num" /etc/iproute2/rt_tables; then
...
main() {
    ...
    while true; do
        ...
        # 检测旁路由是否在线
        if ping -c 2 "$SIDE_ROUTER_IP" > /dev/null; then
            # 如果在线，确保路由存在
            ip route show table "$TABLE_ID" | grep -q "default" || {
                ip route add default via "$GATEWAY" dev "$INTERFACE" table "$TABLE_ID"
                logger "旁路由在线，已恢复 table $TABLE_ID 的默认路由"
            }
        else
            # 如果离线，删除路由
            ip route del default via "$GATEWAY" dev "$INTERFACE" table "$TABLE_ID" 2>/dev/null
            logger "旁路由离线，已移除 table $TABLE_ID 的默认路由"
        fi
