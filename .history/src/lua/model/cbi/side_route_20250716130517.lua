...
-- 替换为 GridSection 来显示设备列表
local device_section = m:section(cbi.SimpleSection, "device", translate("Device List"))
...
local device_table =  luci.cbi.Table()

-- 添加表头
local header = device_table:add_row(true)  -- true 表示这是表头行
header:cell(luci.cbi.DummyValue, nil, translate("Enable"))
header:cell(luci.cbi.DummyValue, nil, translate("Name"))
header:cell(luci.cbi.DummyValue, nil, translate("IP Address"))
...
for k, v in pairs(side_route_config) do
    if v[".type"] == "device" then
        local row = device_table:add_row()
        row:cell(cbi.Value({ section = k, option = "name", map = m }, v.name))
        row:cell(cbi.Value({ section = k, option = "ip", map = m }, v.ip))
        row:cell(cbi.Flag({ section = k, option = "enable", map = m }, v.enable))
    end
end
device_section.render = function(self, node)
    device_table:render(node)
end
