...
name_opt.validate = function(self, value, section)
    ...
    sys.exec(string.format('logger -t LUCI_DEBUG "CBI: value:%s section=%s"', value,tostring(section)))
    -- 如不是临时节点，不进行唯一性校验
...
ip_opt.validate = function(self, value, section)
    ...
    sys.exec(string.format('logger -t LUCI_DEBUG "CBI: section=%s"', tostring(section)))
