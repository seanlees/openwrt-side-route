...
-- ========== 全局配置 ==========
local g = m:section(TypedSection, "global","global", translate("Global Settings"))
g.anonymous = false
...
function apply.write(self, section)
    ...
    -- 收集当前配置值
    for _, option in pairs(g.options) do
        if option.option ~= "_apply" and option.option ~= "_reset" then
            current_config[option.option] = option:formvalue(section)
        end
    end
    
    -- 比较配置是否变化
    for key, value in pairs(current_config) do
        if tostring(original_config[key]) ~= tostring(value) then
            has_changes = true
            break
        end
    end
    ...
    if has_changes then
        -- 显式保存表单更改到内存
        uci:set("side-route", "global", "route_table_name", current_config.route_table_name)
        uci:set("side-route", "global", "route_side_ip", current_config.route_side_ip)
        uci:set("side-route", "global", "route_interface", current_config.route_interface)
        uci:set("side-route", "global", "route_mark", current_config.route_mark)
...
function reset.write(self, section)
    ...
    for _, option in ipairs(g.options) do
        if option.option ~= "_apply" and option.option ~= "_reset" then
            if original_config[option.option] then
                uci:revert("side-route", "global", option.option)
            end
        end
    end
