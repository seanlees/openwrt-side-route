...
local device_section = m:section(cbi.GridSection, "device", translate("Device List"))
device_section.addremove = false -- 禁止通过界面添加或删除设备
device_section.anonymous = true  -- 不显示节名称

-- 添加表格列
device_section:option(cbi.Flag, "enable", translate("Enable"))
device_section:option(cbi.Value, "name", translate("Name"))
device_section:option(cbi.Value, "ip", translate("IP Address"))
...
for k, v in pairs(side_route_config) do
    if v[".type"] == "device" then
        local dev = device_section:tab("device_" .. k, v.name)
        dev.fields = {
            enable = { value = v.enable },
            name = { value = v.name },
            ip = { value = v.ip }
        }
    end
