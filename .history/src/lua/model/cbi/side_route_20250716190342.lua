...
-- ==== 自定义添加设备时的 SID 生成逻辑 ====
function device_section.create(self)
    -- 获取用户输入的 name 字段
    local name = self.map:get(self.config, self.section, "name")
    local sid

    if name and name ~= "" then
        -- 使用 name 转换为合法的 SID（只保留字母数字和下划线）
        sid = name:lower():gsub("[^a-z0-9_]", "_")
    else
        -- 如果 name 为空，使用默认前缀 + 随机数
        sid = "device_" .. os.time()
    end

    -- 确保 SID 唯一
    local i = 1
    local original_sid = sid
    while self.map:get(sid) do
        sid = original_sid .. "_" .. i
        i = i + 1
    end

    -- 创建命名节
    return self.map:create(self.config, sid, "device")
end
