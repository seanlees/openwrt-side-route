...
local device_table = Table()
-- 添加表头
device_table.rowcolors = true -- 开启行颜色交替
device_table:row_header {
    translate("Name"),
...
uci:foreach("side-route", "device", function(dev)
    ...
    -- Enable 字段
    local enable_cell = Value()
    enable_cell.readonly = true
    enable_cell.cfgvalue = function()
        return dev.enable == "1" and "Enabled" or "Disabled"
    end
    r:cell(enable_cell, dev[".name"])

    -- Name 字段
    local name_cell = Value()
    name_cell.readonly = true
    name_cell.cfgvalue = function()
        return dev.name or "<empty>"
    end
    r:cell(name_cell, dev[".name"])

    -- IP 地址字段
    local ip_cell = Value()
    ip_cell.readonly = true
    ip_cell.cfgvalue = function()
        return dev.ip or "<none>"
    end
    r:cell(ip_cell, dev[".name]")
end)

-- 把表格渲染到 SimpleSection 中
ss.render = function(self, node)
    t:render(node)
end
