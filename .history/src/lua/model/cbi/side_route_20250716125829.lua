...
local device_table = device_section:add_child(cbi.Table)
...
for k, v in pairs(side_route_config) do
    if v[".type"] == "device" then
        ...
        row:cell(cbi.DummyValue({ section = k, option = "name", map = m }, v.name))
        row:cell(cbi.DummyValue({ section = k, option = "ip", map = m }, v.ip))
    end
end
