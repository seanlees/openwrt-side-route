...
ip_opt.validate = function(self, value, section)
    if not value or value == "" then
     ...
     sys.exec(string.format('logger -t LUCI_DEBUG "CBI: dev=%s"',util.serialize_data(section)))
    -- 如果是临时节（如 cfg12345），不进行唯一性校验
    if section:sub(1, 3) == "cfg" then
        return value
    end
    ...
    local exists =false
    uci:foreach("side-route", "device", function(dev)
        sys.exec(string.format('logger -t LUCI_DEBUG "CBI: dev=%s"',util.serialize_data(dev)))
        sys.exec(string.format('logger -t LUCI_DEBUG "CBI: value=%s"',value))
        if dev[".ip"] ~= section and dev['ip'] == value then
            exists = true
            return false  -- break loop
        end
    end)
