...
dev_name.validate = function(self, value, section)
    -- 检查是否有重复的 name
    local conflicts = {}
    m:foreach("device", "device", function(s)
        if s[".name"] ~= section and m:get(s[".name"], "name") == value then
            table.insert(conflicts, s[".name"])
        end
    end)

    if #conflicts > 0 then
        return nil, string.format(translate("The device name '%s' is already used in sections: %s"), value, table.concat(conflicts, ", "))
    end
...
ip_opt.validate = function(self, value, section)
    ...
    local exists = false
    local sections =  uci:get_all("side-route","device")
