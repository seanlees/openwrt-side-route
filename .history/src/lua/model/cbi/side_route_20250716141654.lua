...
device_section.addremove = false -- 禁止通过界面添加或删除设备
device_section.anonymous = true  -- 不显示节名称
device_section.template = "cbi/tblsection"

-- 设备名称
device_section:option(Value, "name", translate("Device Name")).rmempty = false

-- IP地址
device_section:option(Value, "ip", translate("IP Address")).datatype = "ipaddr"
...
-- 操作列
-- 正确的操作列实现
local actions = device_section:option(DummyValue, "_actions", translate("Actions"))
actions.template = "cbi/tblsection_actions"  -- 使用LuCI标准操作模板
actions.width = "15%"  -- 设置列宽

-- 添加编辑和删除按钮
actions.render = function(self, section, scope)
    -- 编辑按钮
    local edit_url = luci.dispatcher.build_url("admin/services/side_route/device", section)
    local edit_btn = string.format(
        '<a class="cbi-button cbi-button-edit" href="%s">%s</a>',
        edit_url, translate("Edit")
    )
    
    -- 删除按钮
    local del_btn = string.format(
        '<a class="cbi-button cbi-button-remove" href="%s">%s</a>',
        luci.dispatcher.build_url("admin/services/side_route/delete", section),
        translate("Delete")
    )
