...
function s.filter(self, section)
    ...
    if sid == "-" then
        sys.exec(string.format('logger -t LUCI_DEBUG "CBI: newsection2=%s"',self.newsection))
...
if sid == "-" then
    ...
    sys.exec(string.format('logger -t LUCI_DEBUG "CBI: newsection1=%s"',s.newsection))
    -- 创建临时 section 但不要保存到 UCI
    cursor:set("side-route", "device", s.newsection)
else
