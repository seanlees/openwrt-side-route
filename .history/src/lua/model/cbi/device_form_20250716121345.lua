...
sys.exec(string.format('logger -t LUCI_DEBUG "CBI: redirect_url=%s"', luci.dispatcher.build_url("admin/services/side_route")))

local cancel = s:option(DummyValue, "_cancel", translate("Cancel"))
cancel.template = "cbi/button"
cancel.inputstyle = "button"
cancel.value = translate("Cancel")
cancel.onclick = string.format(
    "location.href='%s'", 
    luci.dispatcher.build_url("admin/services/side_route")
)
