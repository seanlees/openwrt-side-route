...
apply.write = function(self, section)
    ...
    self.map:parse()
    if not self.map.changed then return end
    -- 显式保存表单更改到内存
    self.map:save()
    uci:save("side-route")
...
if not uci:get_all("side-route", "device") then
    sys.exec('logger -t LUCI_DEBUG "side-route config invalid"')
end

local config_data = uci:get_all("side-route", "device") or {}

for k, v in pairs(uci:get_all("side-route", "device")) do
