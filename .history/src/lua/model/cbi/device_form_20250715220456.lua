...
s.dynamic = true  -- 允许动态添加/删除

-- 只显示当前设备
unction s.filter(self, section)
    if sid == "-" then
        return false  -- 新建时不显示已有项
    else
        return section == sid  -- 只显示当前 sid 的设备
    end
end

-- 为添加操作创建新的 section ID
if sid == "-" then
    local temp_sid = "temp_" .. os.time()
    -- 创建临时 section 但不要保存到 UCI
    cursor:set("side-route", temp_sid, "device")
    s.section = s.temp_sid
    s.newsection = true
    s.temporary = true
else
