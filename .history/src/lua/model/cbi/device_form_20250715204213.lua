...
if lan then
    ...
    local gateway = cursor:get("side-route", "global", "route_side_ip")
    if route_ip == gateway then
        device_ip:value(router_ip)
    end
...
function device_ip:validate(value)
    -- value 是一个 table，包含 DynamicList 中的所有值
    if type(value) == "table" and #value > 1 then
        return nil, "只能添加一个值！"
    end
    -- 如果是空，根据你的需求决定是否允许
    if type(value) == "table" and #value == 0 then
        return nil, "请至少添加一个值！" -- 或者 return true 允许为空
    end
    -- 如果只有一个值，返回该值
    if type(value) == "table" and #value == 1 then
        return value[1]
